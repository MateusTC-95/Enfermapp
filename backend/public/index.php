...
if(!empty($data->acao)) {
    ...
    // AÇÃO DE CADASTRO (Nova!)
    if($data->acao == "cadastrar") {
        if($usuario->cadastrar($data)) {
            echo json_encode(["status" => "sucesso", "message" => "Usuário criado com sucesso!"]);
        } else {
            echo json_encode(["status" => "erro", "message" => "Erro ao salvar no banco. Nome de usuário já existe?"]);
        }
    }

} else {
    echo json_encode(["status" => "erro", "message" => "Nenhuma acao definida."]);
}